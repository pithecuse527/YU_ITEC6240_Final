...
# %%
roc_fig, _ = roc_oob_figure(y_true, y_score, title_prefix="ROC — CV on train pool, logistic regression")
...
fig_mda = px.violin(
    ...
    box=True,
    points="all",
    title=f"MDA on held-out test — logistic regression ({EXPERIMENT.mda_n_repeats} shuffle reps / feature)",
)
...
fig_shap = px.violin(
    ...
    points=False,
    title=(
        f"SHAP for P(class=1) — logistic (up to {EXPERIMENT.shap_max_explain} rows; "
