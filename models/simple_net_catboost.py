...
for tr_idx, va_idx in skf.split(X_pool, y_pool):
    ...
    clf.fit(X_tr, y_tr)
    y_true.extend(y_va)
    y_pred.extend(clf.predict(X_va).astype(int).ravel())
...
# %%
roc_fig, _ = roc_oob_figure(y_true, y_score, title_prefix="ROC — CV on train pool, CatBoost")
...
fig_mda = px.violin(
    ...
    box=True,
    points="all",
    title=f"MDA on held-out test — CatBoost ({EXPERIMENT.mda_n_repeats} shuffle reps / feature)",
)
...
fig_shap = px.violin(
    ...
    points=False,
    title=(
        f"SHAP for P(class=1) — CatBoost (up to {EXPERIMENT.shap_max_explain} rows; "
